...
public class WeatherAdaptor {
	...
	public static boolean getWeather(String city){
		boolean bufferStr = false ;
		try{ 
				bufferStr=WeatherAdaptor.getWeatherData(city);
				WeatherPicture.doConverter();
		}catch(Throwable e){
			e.printStackTrace();
		}
		return bufferStr;
	}
	private static boolean getWeatherData(String city) throws IOException,Throwable{
	 ...
	 public static List<Weather> getWeatherFromK780(String city)throws Exception{
			...
			for (int i = 0; i < array.length(); i++) {
				...
				WeatherPicture.setPictures(weather);
